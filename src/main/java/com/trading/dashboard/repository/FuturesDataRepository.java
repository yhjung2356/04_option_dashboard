...
public interface FuturesDataRepository extends JpaRepository<FuturesData, Long> {
    ...
    @Query("SELECT f FROM FuturesData f ORDER BY f.volume DESC")
    List<FuturesData> findTopByVolumeDesc();
    
    @Query("SELECT f FROM FuturesData f ORDER BY f.tradingValue DESC")
    List<FuturesData> findTopByTradingValueDesc();
