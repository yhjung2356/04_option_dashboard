...
- [x] 자동 재연결 로직 (지수 백오프)
- [x] 로그 레벨 최적화 (상세 로그 INFO → DEBUG, 요약만 INFO 유지)
...
- [x] TypeScript 컴파일 오류 수정 (사용하지 않는 import 제거)
- [x] pom.xml 빌드 시스템 개선 (프로퍼티 기반 빌드 제어)
- [x] deploy.sh 업데이트 (프론트엔드/백엔드 빌드 분리, clean 제거)
- [x] DEPLOYMENT.md 최신화 (빌드 명령어 업데이트)
...
1. ✅ Greeks 값 0.000 문제 해결 (MCP 서버 활용하여 필드명 확인)
2. ✅ WebSocket 연결 상태 인디케이터 추가 (Toast 알림 포함)
3. ✅ 데이터 새로고침 기능 구현 (F5 단축키, 마지막 업데이트 시간)
4. ✅ 전역 에러 바운더리 설정
5. ✅ API 타임아웃 처리 개선 (30초, 3회 재시도, Exponential Backoff)
6. ✅ 로그 레벨 최적화 (상세 로그 DEBUG로 변경, 프로덕션 로그 정리)
7. ✅ 빌드 시스템 개선 (pom.xml 프로퍼티, JVM 크래시 방지)
8. ✅ 배포 스크립트 업데이트 (deploy.sh, DEPLOYMENT.md)
